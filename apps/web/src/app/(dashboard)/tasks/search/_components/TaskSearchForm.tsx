...
interface TaskSearchFormProps {
  ...
  /** 子メンバー一覧（親ロールのみ担当者フィルタを表示するために使用） */
  childMembers?: GroupMember[];
  role?: string | null;
}

// タスク検索フォームコンポーネント（クライアントコンポーネント）
export function TaskSearchForm({ defaultValues, childMembers = [], role }: TaskSearchFormProps) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (assigneeId && typeof assigneeId === 'string' && assigneeId !== '') {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border p-6 mb-6">
      <div className={`grid grid-cols-1 gap-4 ${isParent && childMembers.length > 0 ? 'sm:grid-cols-4' : 'sm:grid-cols-3'}`}>
        {/* タスク名キーワード入力 */}
        <div>
