...
## 修正・仕様変更（fix/FUN-TASK-006）

### フロントエンド（Next.js）

- [x] 🟡 SCR-TASK-001: タスクカレンダーに日付選択機能を追加し、選択日のタスク一覧をカレンダー下部のパネルに表示（仕様変更）
  - 日付セルのクリックで選択状態をトグル（セル枠を青くハイライト）
  - 月ナビゲーション時に選択状態をリセット
  - 選択日の単日タスク（due_date / start_time 一致）と複数日タスク（期間が当日をまたぐもの）をパネルに一覧表示
  - ステータスフィルタの選択に連動して表示タスクを絞り込む
  - 各行クリックでタスク詳細（`/tasks/:id`）へ遷移。0件時は「この日のタスクはありません」を表示
  - フロントのクライアントサイドフィルタで対応（APIの追加変更なし）
